!> AMR collocated compressible solver class - GPU (OpenMP target offload) variant
!> Mirrors src/amrsolvers/amrcomp_class.f90; kept as a separate module name so both
!> can coexist in a build. Single-species only (ns=1).
module amrcomp_gpu_class
...
contains
   ...
   subroutine initialize(this,amr,name)
      ...
      use device_eos,       only: dev_eos,device_eos_init_from_material,device_eos_push
      ...
      ! This GPU variant only supports a single species -- the device EOS path and the
      ! offloaded flux kernels both assume ns=1 (no Y transport, no interdiffusion)
      if (this%mat%ns.gt.1) call die('[amrcomp initialize] GPU variant supports single-species materials only')

      ! Flatten the polymorphic material into the device-resident EOS parameter block
      call device_eos_init_from_material(mat=this%mat,dev_eos=dev_eos)
      call device_eos_push()
   ...
   !> Offloaded: array-section sum() intrinsics scalarized, rho_floor hoisted
   subroutine get_face_velocity(this)
      implicit none
      class(amrcomp), intent(inout) :: this
      integer :: lvl,i,j,k,ilo,ihi,jlo,jhi,klo,khi
      type(amrex_mfiter) :: mfi
      type(amrex_box) :: fbx
      real(WP), dimension(:,:,:,:), contiguous, pointer :: pQ,pU,pV,pW
      real(WP) :: rho_floor
      ! Hoist scalars off the derived type -- this% is not accessible inside target regions
      rho_floor=this%rho_floor
      ! Traverse levels
      do lvl=0,this%amr%clvl()
         ...
         do while (mfi%next())
            ...
            ilo=fbx%lo(1); ihi=fbx%hi(1); jlo=fbx%lo(2); jhi=fbx%hi(2); klo=fbx%lo(3); khi=fbx%hi(3)
            !$omp target teams distribute parallel do collapse(3) &
            !$omp&  has_device_addr(pQ, pU) firstprivate(rho_floor)
            do k=klo,khi; do j=jlo,jhi; do i=ilo,ihi
                     pU(i,j,k,1)=(pQ(i-1,j,k,2)+pQ(i,j,k,2))/max(pQ(i-1,j,k,1)+pQ(i,j,k,1),rho_floor)
            ...
            ilo=fbx%lo(1); ihi=fbx%hi(1); jlo=fbx%lo(2); jhi=fbx%hi(2); klo=fbx%lo(3); khi=fbx%hi(3)
            !$omp target teams distribute parallel do collapse(3) &
            !$omp&  has_device_addr(pQ, pV) firstprivate(rho_floor)
            do k=klo,khi; do j=jlo,jhi; do i=ilo,ihi
                     pV(i,j,k,1)=(pQ(i,j-1,k,3)+pQ(i,j,k,3))/max(pQ(i,j-1,k,1)+pQ(i,j,k,1),rho_floor)
                  end do; end do; end do
            !$omp end target teams distribute parallel do
            ! Get Z-face velocity
            ...
            ilo=fbx%lo(1); ihi=fbx%hi(1); jlo=fbx%lo(2); jhi=fbx%hi(2); klo=fbx%lo(3); khi=fbx%hi(3)
            !$omp target teams distribute parallel do collapse(3) &
            !$omp&  has_device_addr(pQ, pW) firstprivate(rho_floor)
            do k=klo,khi; do j=jlo,jhi; do i=ilo,ihi
                     pW(i,j,k,1)=(pQ(i,j,k-1,4)+pQ(i,j,k,4))/max(pQ(i,j,k-1,1)+pQ(i,j,k,1),rho_floor)
                  end do; end do; end do
            !$omp end target teams distribute parallel do
         end do
   ...
   subroutine add_pressure(this,scale,phi,mask,gravity)
      ...
      integer :: lvl,i,j,k,ilo,ihi,jlo,jhi,klo,khi
      ! Hoisted state -- none of this%, present(), or geom() is reachable inside a target region
      real(WP) :: rho_floor,gx,gy,gz
      logical :: use_phi,has_mask
      logical :: fix_xlo,fix_xhi,fix_ylo,fix_yhi,fix_zlo,fix_zhi
      integer :: dlo1,dhi1,dlo2,dhi2,dlo3,dhi3
      ! Hoist scalars and optional-argument presence
      rho_floor=this%rho_floor
      use_phi=present(phi)
      has_mask=present(mask)
      ! Fold gravity into unconditional addends (zero when absent) to drop a branch from the kernel
      gx=0.0_WP; gy=0.0_WP; gz=0.0_WP
      if (present(gravity)) then; gx=gravity(1); gy=gravity(2); gz=gravity(3); end if
      ! Collapse the periodicity and BC-type tests into one flag per face
      fix_xlo=(.not.this%amr%xper).and.(this%U%lo_bc(1,1).ne.amrex_bc_reflect_odd)
      fix_xhi=(.not.this%amr%xper).and.(this%U%hi_bc(1,1).ne.amrex_bc_reflect_odd)
      fix_ylo=(.not.this%amr%yper).and.(this%V%lo_bc(2,1).ne.amrex_bc_reflect_odd)
      fix_yhi=(.not.this%amr%yper).and.(this%V%hi_bc(2,1).ne.amrex_bc_reflect_odd)
      fix_zlo=(.not.this%amr%zper).and.(this%W%lo_bc(3,1).ne.amrex_bc_reflect_odd)
      fix_zhi=(.not.this%amr%zper).and.(this%W%hi_bc(3,1).ne.amrex_bc_reflect_odd)
      ...
      if (present(phi)) then
         ...
         do lvl=0,this%amr%clvl()
            ...
            do while (mfi%next())
               ...
               ilo=bx%lo(1); ihi=bx%hi(1); jlo=bx%lo(2); jhi=bx%hi(2); klo=bx%lo(3); khi=bx%hi(3)
               ! Compute pressure gradient at faces
               !$omp target teams distribute parallel do collapse(3) &
               !$omp&  has_device_addr(pP, pQ, pFx) firstprivate(dxi, rho_floor, gx)
               do k=klo,khi; do j=jlo,jhi; do i=ilo,ihi
                        pFx(i,j,k,1)=-2.0_WP*(pP(i,j,k,1)-pP(i-1,j,k,1))*dxi/(max(pQ(i-1,j,k,1),rho_floor)+max(pQ(i,j,k,1),rho_floor))+gx
                     end do; end do; end do
               !$omp end target teams distribute parallel do
               bx=mfi%nodaltilebox(2)
               ilo=bx%lo(1); ihi=bx%hi(1); jlo=bx%lo(2); jhi=bx%hi(2); klo=bx%lo(3); khi=bx%hi(3)
               !$omp target teams distribute parallel do collapse(3) &
               !$omp&  has_device_addr(pP, pQ, pFy) firstprivate(dyi, rho_floor, gy)
               do k=klo,khi; do j=jlo,jhi; do i=ilo,ihi
                        pFy(i,j,k,1)=-2.0_WP*(pP(i,j,k,1)-pP(i,j-1,k,1))*dyi/(max(pQ(i,j-1,k,1),rho_floor)+max(pQ(i,j,k,1),rho_floor))+gy
                     end do; end do; end do
               !$omp end target teams distribute parallel do
               bx=mfi%nodaltilebox(3)
               ilo=bx%lo(1); ihi=bx%hi(1); jlo=bx%lo(2); jhi=bx%hi(2); klo=bx%lo(3); khi=bx%hi(3)
               !$omp target teams distribute parallel do collapse(3) &
               !$omp&  has_device_addr(pP, pQ, pFz) firstprivate(dzi, rho_floor, gz)
               do k=klo,khi; do j=jlo,jhi; do i=ilo,ihi
                        pFz(i,j,k,1)=-2.0_WP*(pP(i,j,k,1)-pP(i,j,k-1,1))*dzi/(max(pQ(i,j,k-1,1),rho_floor)+max(pQ(i,j,k,1),rho_floor))+gz
                     end do; end do; end do
               !$omp end target teams distribute parallel do
            end do
            call this%amr%mfiter_destroy(mfi)
      ...
      do lvl=0,this%amr%clvl()
         ! Get mesh size
         dxi=1.0_WP/this%amr%dx(lvl); dyi=1.0_WP/this%amr%dy(lvl); dzi=1.0_WP/this%amr%dz(lvl)
         ! Get this level's domain extent (used by the non-periodic BC fix below)
         dlo1=this%amr%geom(lvl)%domain%lo(1); dhi1=this%amr%geom(lvl)%domain%hi(1)
         dlo2=this%amr%geom(lvl)%domain%lo(2); dhi2=this%amr%geom(lvl)%domain%hi(2)
         dlo3=this%amr%geom(lvl)%domain%lo(3); dhi3=this%amr%geom(lvl)%domain%hi(3)
         ...
         do while (mfi%next())
            ...
            pV=>this%V%mf(lvl)%dataptr(mfi)
            pW=>this%W%mf(lvl)%dataptr(mfi)
            ! Optional inputs still need a valid device address to appear in has_device_addr,
            ! so alias them onto pQ when absent. Guarded by has_mask/use_phi, never read.
            if (has_mask) then; pMask=>mask%mf(lvl)%dataptr(mfi); else; pMask=>pQ; end if
            if (use_phi) pPold=>pQ
            ! Get tilebox
            bx=mfi%tilebox()
            ilo=bx%lo(1); ihi=bx%hi(1); jlo=bx%lo(2); jhi=bx%hi(2); klo=bx%lo(3); khi=bx%hi(3)
            !$omp target teams distribute parallel do collapse(3)                        &
            !$omp&  has_device_addr(pQ, pP, pPold, pU, pV, pW, pFx, pFy, pFz, pMask)     &
            !$omp&  firstprivate(scale, dxi, dyi, dzi, use_phi, has_mask,                &
            !$omp&               fix_xlo, fix_xhi, fix_ylo, fix_yhi, fix_zlo, fix_zhi,   &
            !$omp&               dlo1, dhi1, dlo2, dhi2, dlo3, dhi3)                     &
            !$omp&  private(coeff, crossterm)
            do k=klo,khi; do j=jlo,jhi; do i=ilo,ihi
                     pQ(i,j,k,2)=pQ(i,j,k,2)+scale*pQ(i,j,k,1)*0.5_WP*(pFx(i,j,k,1)+pFx(i+1,j,k,1))
                     pQ(i,j,k,3)=pQ(i,j,k,3)+scale*pQ(i,j,k,1)*0.5_WP*(pFy(i,j,k,1)+pFy(i,j+1,k,1))
                     pQ(i,j,k,4)=pQ(i,j,k,4)+scale*pQ(i,j,k,1)*0.5_WP*(pFz(i,j,k,1)+pFz(i,j,k+1,1))
                     coeff=1.0_WP; if (has_mask) coeff=pMask(i,j,k,1)
                     crossterm=0.0_WP; if (.not.use_phi) crossterm=-coeff*scale**2*pPold(i,j,k,1)*(dxi*(pFx(i+1,j,k,1)-pFx(i,j,k,1))+dyi*(pFy(i,j+1,k,1)-pFy(i,j,k,1))+dzi*(pFz(i,j,k+1,1)-pFz(i,j,k,1)))
                     ...
                     if (fix_xlo.and.i.eq.dlo1) pQ(i,j,k,2)=pQ(i,j,k,2)+scale*0.25_WP*(pQ(i,j,k,1)+pQ(i+1,j,k,1))*pFx(i+1,j,k,1)
                     if (fix_xhi.and.i.eq.dhi1) pQ(i,j,k,2)=pQ(i,j,k,2)+scale*0.25_WP*(pQ(i-1,j,k,1)+pQ(i,j,k,1))*pFx(i  ,j,k,1)
                     if (fix_ylo.and.j.eq.dlo2) pQ(i,j,k,3)=pQ(i,j,k,3)+scale*0.25_WP*(pQ(i,j,k,1)+pQ(i,j+1,k,1))*pFy(i,j+1,k,1)
                     if (fix_yhi.and.j.eq.dhi2) pQ(i,j,k,3)=pQ(i,j,k,3)+scale*0.25_WP*(pQ(i,j-1,k,1)+pQ(i,j,k,1))*pFy(i,j  ,k,1)
                     if (fix_zlo.and.k.eq.dlo3) pQ(i,j,k,4)=pQ(i,j,k,4)+scale*0.25_WP*(pQ(i,j,k,1)+pQ(i,j,k+1,1))*pFz(i,j,k+1,1)
                     if (fix_zhi.and.k.eq.dhi3) pQ(i,j,k,4)=pQ(i,j,k,4)+scale*0.25_WP*(pQ(i,j,k-1,1)+pQ(i,j,k,1))*pFz(i,j,k  ,1)
                  end do; end do; end do
            !$omp end target teams distribute parallel do
         end do
         call this%amr%mfiter_destroy(mfi)
   ...
   !> Offloaded: the polymorphic this%mat%get_*() calls are replaced by the flat,
   !> device-callable dispatch in device_eos, reading the device-resident dev_eos.
   subroutine get_primitive(this,Q)
      use device_eos, only: dev_eos,device_get_p_from_rho_e,device_get_c_from_rho_e,device_get_T_from_rho_e
      use messager,   only: die
      implicit none
      class(amrcomp), intent(inout) :: this
      ...
      real(WP) :: irho,rho_floor
      ! Single-species: composition is trivially y=[1]. Kept as a rank-1 array because
      ! the device EOS interface takes y as an assumed-shape dummy.
      ...
      ! Hoist scalars off the derived type -- this% is not accessible inside target regions
      rho_floor=this%rho_floor; y(1)=1.0_WP
      ! Loop over levels
      do lvl=0,this%amr%clvl()
         call this%amr%mfiter_build(lvl,mfi)
         do while (mfi%next())
            ...
            ilo=bx%lo(1); ihi=bx%hi(1); jlo=bx%lo(2); jhi=bx%hi(2); klo=bx%lo(3); khi=bx%hi(3)
            !$omp target teams distribute parallel do collapse(3) &
            !$omp&  has_device_addr(pQ, pUVW, pI, pP, pT, pC)     &
            !$omp&  firstprivate(rho_floor, y)                    &
            !$omp&  private(irho)
            do k=klo,khi; do j=jlo,jhi; do i=ilo,ihi
                     ! Compute velocity from momentum
                     irho=1.0_WP/max(pQ(i,j,k,1),rho_floor)
                     ...
                     pP(i,j,k,1)=device_get_p_from_rho_e(dev_eos,pQ(i,j,k,1),pI(i,j,k,1),y)
                     ! Compute speed of sound via EoS from (rho,e) -- avoids the ill-conditioned (p,rho) flash
                     pC(i,j,k,1)=device_get_c_from_rho_e(dev_eos,pQ(i,j,k,1),pI(i,j,k,1),y)
                     ! Compute temperature via EoS from (rho,e)
                     pT(i,j,k,1)=device_get_T_from_rho_e(dev_eos,pQ(i,j,k,1),pI(i,j,k,1),y)
                  end do; end do; end do
            !$omp end target teams distribute parallel do
         end do
         call this%amr%mfiter_destroy(mfi)
   ...
   pure real(WP) function device_weno_weight(ratio) result(ww)
      !$omp declare target
      implicit none
      real(WP), intent(in) :: ratio
      real(WP), parameter :: lambda=0.13_WP
      real(WP), parameter :: delta=0.01_WP
      ww = (1.0_WP-tanh((ratio-lambda)/delta))/3.0_WP + &
         (1.0_WP-tanh((ratio-1.0_WP/lambda)/delta))/6.0_WP
   end function device_weno_weight
   ...
   subroutine get_dQdt(this,dQdt)
      ...
      compute_fluxes: block
         integer :: lvl,i,j,k,n,ilo,ihi,jlo,jhi,klo,khi
         type(amrex_mfiter) :: mfi
         type(amrex_box) :: fbx
         ...
         do lvl=0,this%amr%clvl()
            ...
            do while (mfi%next())
               ...
               ilo=fbx%lo(1); ihi=fbx%hi(1); jlo=fbx%lo(2); jhi=fbx%hi(2); klo=fbx%lo(3); khi=fbx%hi(3)
               !$omp target teams distribute parallel do collapse(3) &
               !$omp&  has_device_addr(pU, pQ, pUVW, pI, pT, pVisc, pBeta, pDiff, pP, pFx) &
               !$omp&  firstprivate(dxi, dyi, dzi)                &
               !$omp&  private(w, wenop, wenom, gradU, div)
               do k=klo,khi; do j=jlo,jhi; do i=ilo,ihi
                        ! WENO mass flux
                        w=device_weno_weight((abs(pQ(i-1,j,k,1)-pQ(i-2,j,k,1))+eps)/(abs(pQ(i,j,k,1)-pQ(i-1,j,k,1))+eps))
                        wenop(-2)=-0.5_WP*w; wenop(-1)=0.5_WP*(1.0_WP+2.0_WP*w); wenop(0)=0.5_WP*(1.0_WP-w)
                        w=device_weno_weight((abs(pQ(i+1,j,k,1)-pQ(i  ,j,k,1))+eps)/(abs(pQ(i,j,k,1)-pQ(i-1,j,k,1))+eps))
                        wenom(-1)=0.5_WP*(1.0_WP-w); wenom(0)=0.5_WP*(1.0_WP+2.0_WP*w); wenom(1)=-0.5_WP*w
                        pFx(i,j,k,1)=-0.5_WP*(pU(i,j,k,1)+abs(pU(i,j,k,1)))*((wenop(-2)*pQ(i-2,j,k,1)+wenop(-1)*pQ(i-1,j,k,1)+wenop(0)*pQ(i  ,j,k,1))) &
                           -0.5_WP*(pU(i,j,k,1)-abs(pU(i,j,k,1)))*((wenom(-1)*pQ(i-1,j,k,1)+wenom( 0)*pQ(i  ,j,k,1)+wenom(1)*pQ(i+1,j,k,1)))
                        ! Momentum fluxes
                        pFx(i,j,k,2)=pFx(i,j,k,1)*0.5_WP*(pUVW(i-1,j,k,1)+pUVW(i,j,k,1))
                        pFx(i,j,k,3)=pFx(i,j,k,1)*0.5_WP*(pUVW(i-1,j,k,2)+pUVW(i,j,k,2))
                        pFx(i,j,k,4)=pFx(i,j,k,1)*0.5_WP*(pUVW(i-1,j,k,3)+pUVW(i,j,k,3))
                        ! WENO internal energy flux
                        w=device_weno_weight((abs(pI(i-1,j,k,1)-pI(i-2,j,k,1))+eps)/(abs(pI(i,j,k,1)-pI(i-1,j,k,1))+eps))
                        wenop(-2)=-0.5_WP*w; wenop(-1)=0.5_WP*(1.0_WP+2.0_WP*w); wenop(0)=0.5_WP*(1.0_WP-w)
                        w=device_weno_weight((abs(pI(i+1,j,k,1)-pI(i  ,j,k,1))+eps)/(abs(pI(i,j,k,1)-pI(i-1,j,k,1))+eps))
                        wenom(-1)=0.5_WP*(1.0_WP-w); wenom(0)=0.5_WP*(1.0_WP+2.0_WP*w); wenom(1)=-0.5_WP*w
                        pFx(i,j,k,5)=0.5_WP*(pFx(i,j,k,1)-abs(pFx(i,j,k,1)))*(wenop(-2)*pI(i-2,j,k,1)+wenop(-1)*pI(i-1,j,k,1)+wenop(0)*pI(i  ,j,k,1)) &
                        &           +0.5_WP*(pFx(i,j,k,1)+abs(pFx(i,j,k,1)))*(wenom(-1)*pI(i-1,j,k,1)+wenom( 0)*pI(i  ,j,k,1)+wenom(1)*pI(i+1,j,k,1))
                        ...
                        pFx(i,j,k,2)=pFx(i,j,k,2)+0.5_WP*(pVisc(i-1,j,k,1)+pVisc(i,j,k,1))*(gradU(1,1)+gradU(1,1))+0.5_WP*(pBeta(i-1,j,k,1)+pBeta(i,j,k,1)-2.0_WP/3.0_WP*(pVisc(i-1,j,k,1)+pVisc(i,j,k,1)))*div
                        pFx(i,j,k,3)=pFx(i,j,k,3)+0.5_WP*(pVisc(i-1,j,k,1)+pVisc(i,j,k,1))*(gradU(2,1)+gradU(1,2))
                        pFx(i,j,k,4)=pFx(i,j,k,4)+0.5_WP*(pVisc(i-1,j,k,1)+pVisc(i,j,k,1))*(gradU(3,1)+gradU(1,3))
                        ! Heat diffusion flux
                        pFx(i,j,k,5)=pFx(i,j,k,5)+0.5_WP*(pDiff(i-1,j,k,1)+pDiff(i,j,k,1))*dxi*(pT(i,j,k,1)-pT(i-1,j,k,1))
                     end do; end do; end do
               !$omp end target teams distribute parallel do
               ! Y-fluxes
               fbx=mfi%nodaltilebox(2)
               ilo=fbx%lo(1); ihi=fbx%hi(1); jlo=fbx%lo(2); jhi=fbx%hi(2); klo=fbx%lo(3); khi=fbx%hi(3)
               !$omp target teams distribute parallel do collapse(3) &
               !$omp&  has_device_addr(pV, pQ, pUVW, pI, pT, pVisc, pBeta, pDiff, pP, pFy) &
               !$omp&  firstprivate(dxi, dyi, dzi)                &
               !$omp&  private(w, wenop, wenom, gradU, div)
               do k=klo,khi; do j=jlo,jhi; do i=ilo,ihi
                        ! WENO mass flux
                        w=device_weno_weight((abs(pQ(i,j-1,k,1)-pQ(i,j-2,k,1))+eps)/(abs(pQ(i,j,k,1)-pQ(i,j-1,k,1))+eps))
                        wenop(-2)=-0.5_WP*w; wenop(-1)=0.5_WP*(1.0_WP+2.0_WP*w); wenop(0)=0.5_WP*(1.0_WP-w)
                        w=device_weno_weight((abs(pQ(i,j+1,k,1)-pQ(i,j  ,k,1))+eps)/(abs(pQ(i,j,k,1)-pQ(i,j-1,k,1))+eps))
                        wenom(-1)=0.5_WP*(1.0_WP-w); wenom(0)=0.5_WP*(1.0_WP+2.0_WP*w); wenom(1)=-0.5_WP*w
                        pFy(i,j,k,1)=-0.5_WP*(pV(i,j,k,1)+abs(pV(i,j,k,1)))*((wenop(-2)*pQ(i,j-2,k,1)+wenop(-1)*pQ(i,j-1,k,1)+wenop(0)*pQ(i,j  ,k,1))) &
                        &            -0.5_WP*(pV(i,j,k,1)-abs(pV(i,j,k,1)))*((wenom(-1)*pQ(i,j-1,k,1)+wenom( 0)*pQ(i,j  ,k,1)+wenom(1)*pQ(i,j+1,k,1)))
                        ! Momentum fluxes
                        pFy(i,j,k,2)=pFy(i,j,k,1)*0.5_WP*(pUVW(i,j-1,k,1)+pUVW(i,j,k,1))
                        pFy(i,j,k,3)=pFy(i,j,k,1)*0.5_WP*(pUVW(i,j-1,k,2)+pUVW(i,j,k,2))
                        pFy(i,j,k,4)=pFy(i,j,k,1)*0.5_WP*(pUVW(i,j-1,k,3)+pUVW(i,j,k,3))
                        ! WENO internal energy flux
                        w=device_weno_weight((abs(pI(i,j-1,k,1)-pI(i,j-2,k,1))+eps)/(abs(pI(i,j,k,1)-pI(i,j-1,k,1))+eps))
                        wenop(-2)=-0.5_WP*w; wenop(-1)=0.5_WP*(1.0_WP+2.0_WP*w); wenop(0)=0.5_WP*(1.0_WP-w)
                        w=device_weno_weight((abs(pI(i,j+1,k,1)-pI(i,j  ,k,1))+eps)/(abs(pI(i,j,k,1)-pI(i,j-1,k,1))+eps))
                        wenom(-1)=0.5_WP*(1.0_WP-w); wenom(0)=0.5_WP*(1.0_WP+2.0_WP*w); wenom(1)=-0.5_WP*w
                        pFy(i,j,k,5)=0.5_WP*(pFy(i,j,k,1)-abs(pFy(i,j,k,1)))*(wenop(-2)*pI(i,j-2,k,1)+wenop(-1)*pI(i,j-1,k,1)+wenop(0)*pI(i,j  ,k,1)) &
                        &           +0.5_WP*(pFy(i,j,k,1)+abs(pFy(i,j,k,1)))*(wenom(-1)*pI(i,j-1,k,1)+wenom( 0)*pI(i,j  ,k,1)+wenom(1)*pI(i,j+1,k,1))
                        ...
                        pFy(i,j,k,2)=pFy(i,j,k,2)+0.5_WP*(pVisc(i,j-1,k,1)+pVisc(i,j,k,1))*(gradU(1,2)+gradU(2,1))
                        pFy(i,j,k,3)=pFy(i,j,k,3)+0.5_WP*(pVisc(i,j-1,k,1)+pVisc(i,j,k,1))*(gradU(2,2)+gradU(2,2))+0.5_WP*(pBeta(i,j-1,k,1)+pBeta(i,j,k,1)-2.0_WP/3.0_WP*(pVisc(i,j-1,k,1)+pVisc(i,j,k,1)))*div
                        pFy(i,j,k,4)=pFy(i,j,k,4)+0.5_WP*(pVisc(i,j-1,k,1)+pVisc(i,j,k,1))*(gradU(3,2)+gradU(2,3))
                        ! Heat diffusion flux
                        pFy(i,j,k,5)=pFy(i,j,k,5)+0.5_WP*(pDiff(i,j-1,k,1)+pDiff(i,j,k,1))*dyi*(pT(i,j,k,1)-pT(i,j-1,k,1))
                     end do; end do; end do
               !$omp end target teams distribute parallel do
               ! Z-fluxes
               fbx=mfi%nodaltilebox(3)
               ilo=fbx%lo(1); ihi=fbx%hi(1); jlo=fbx%lo(2); jhi=fbx%hi(2); klo=fbx%lo(3); khi=fbx%hi(3)
               !$omp target teams distribute parallel do collapse(3) &
               !$omp&  has_device_addr(pW, pQ, pUVW, pI, pT, pVisc, pBeta, pDiff, pP, pFz) &
               !$omp&  firstprivate(dxi, dyi, dzi)                &
               !$omp&  private(w, wenop, wenom, gradU, div)
               do k=klo,khi; do j=jlo,jhi; do i=ilo,ihi
                        ! WENO mass flux
                        w=device_weno_weight((abs(pQ(i,j,k-1,1)-pQ(i,j,k-2,1))+eps)/(abs(pQ(i,j,k,1)-pQ(i,j,k-1,1))+eps))
                        wenop(-2)=-0.5_WP*w; wenop(-1)=0.5_WP*(1.0_WP+2.0_WP*w); wenop(0)=0.5_WP*(1.0_WP-w)
                        w=device_weno_weight((abs(pQ(i,j,k+1,1)-pQ(i,j,k  ,1))+eps)/(abs(pQ(i,j,k,1)-pQ(i,j,k-1,1))+eps))
                        wenom(-1)=0.5_WP*(1.0_WP-w); wenom(0)=0.5_WP*(1.0_WP+2.0_WP*w); wenom(1)=-0.5_WP*w
                        pFz(i,j,k,1)=-0.5_WP*(pW(i,j,k,1)+abs(pW(i,j,k,1)))*((wenop(-2)*pQ(i,j,k-2,1)+wenop(-1)*pQ(i,j,k-1,1)+wenop(0)*pQ(i,j,k  ,1))) &
                        &            -0.5_WP*(pW(i,j,k,1)-abs(pW(i,j,k,1)))*((wenom(-1)*pQ(i,j,k-1,1)+wenom( 0)*pQ(i,j,k  ,1)+wenom(1)*pQ(i,j,k+1,1)))
                        ! Momentum fluxes
                        pFz(i,j,k,2)=pFz(i,j,k,1)*0.5_WP*(pUVW(i,j,k-1,1)+pUVW(i,j,k,1))
                        pFz(i,j,k,3)=pFz(i,j,k,1)*0.5_WP*(pUVW(i,j,k-1,2)+pUVW(i,j,k,2))
                        pFz(i,j,k,4)=pFz(i,j,k,1)*0.5_WP*(pUVW(i,j,k-1,3)+pUVW(i,j,k,3))
                        ! WENO internal energy flux
                        w=device_weno_weight((abs(pI(i,j,k-1,1)-pI(i,j,k-2,1))+eps)/(abs(pI(i,j,k,1)-pI(i,j,k-1,1))+eps))
                        wenop(-2)=-0.5_WP*w; wenop(-1)=0.5_WP*(1.0_WP+2.0_WP*w); wenop(0)=0.5_WP*(1.0_WP-w)
                        w=device_weno_weight((abs(pI(i,j,k+1,1)-pI(i,j,k  ,1))+eps)/(abs(pI(i,j,k,1)-pI(i,j,k-1,1))+eps))
                        wenom(-1)=0.5_WP*(1.0_WP-w); wenom(0)=0.5_WP*(1.0_WP+2.0_WP*w); wenom(1)=-0.5_WP*w
                        pFz(i,j,k,5)=0.5_WP*(pFz(i,j,k,1)-abs(pFz(i,j,k,1)))*((wenop(-2)*pI(i,j,k-2,1)+wenop(-1)*pI(i,j,k-1,1)+wenop(0)*pI(i,j,k  ,1))) &
                        &           +0.5_WP*(pFz(i,j,k,1)+abs(pFz(i,j,k,1)))*((wenom(-1)*pI(i,j,k-1,1)+wenom( 0)*pI(i,j,k  ,1)+wenom(1)*pI(i,j,k+1,1)))
                        ...
                        pFz(i,j,k,2)=pFz(i,j,k,2)+0.5_WP*(pVisc(i,j,k-1,1)+pVisc(i,j,k,1))*(gradU(1,3)+gradU(3,1))
                        pFz(i,j,k,3)=pFz(i,j,k,3)+0.5_WP*(pVisc(i,j,k-1,1)+pVisc(i,j,k,1))*(gradU(2,3)+gradU(3,2))
                        pFz(i,j,k,4)=pFz(i,j,k,4)+0.5_WP*(pVisc(i,j,k-1,1)+pVisc(i,j,k,1))*(gradU(3,3)+gradU(3,3))+0.5_WP*(pBeta(i,j,k-1,1)+pBeta(i,j,k,1)-2.0_WP/3.0_WP*(pVisc(i,j,k-1,1)+pVisc(i,j,k,1)))*div
                        ! Heat diffusion flux
                        pFz(i,j,k,5)=pFz(i,j,k,5)+0.5_WP*(pDiff(i,j,k-1,1)+pDiff(i,j,k,1))*dzi*(pT(i,j,k,1)-pT(i,j,k-1,1))
                     end do; end do; end do
               !$omp end target teams distribute parallel do
            end do
            call this%amr%mfiter_destroy(mfi)
      ...
      divergence_and_sources: block
         ...
         do lvl=0,this%amr%clvl()
            ...
            do while (mfi%next())
               ...
               !$omp target teams distribute parallel do collapse(3) &
               !$omp&  has_device_addr(pFx, pFy, pFz, pUVW, pVisc, pBeta, pdQdt) &
               !$omp&  firstprivate(dxi, dyi, dzi, nQ_loc)            &
               !$omp&  private(gradU, div, n)
               do k=klo,khi
                  do j=jlo,jhi
                     do i=ilo,ihi
                        ! Advection
