...
contains
   ...
   subroutine rhs(this,dQdt)
      ...
      real(WP), dimension(:,:,:), allocatable :: VFQ
      integer :: i,j,k,n
      real(WP) :: w,div,vel
      ...
      do k=this%cfg%kmin_,this%cfg%kmax_+1
         do j=this%cfg%jmin_,this%cfg%jmax_+1
            do i=this%cfg%imin_,this%cfg%imax_+1
               ...
               FQx(i,j,k,1)=-0.5_WP*(vel+abs(vel))*sum(wenop*this%Q(i-2:i  ,j,k,1))&
               &            -0.5_WP*(vel-abs(vel))*sum(wenom*this%Q(i-1:i+1,j,k,1))
               ! Centered  mass flux
               ...
               ! Momentum flux and pressure stress
               FQx(i,j,k,3)=FQx(i,j,k,1)*0.5_WP*sum(this%U(i-1:i,j,k))-0.5_WP*sum(this%P(i-1:i,j,k))
               FQx(i,j,k,4)=FQx(i,j,k,1)*0.5_WP*sum(this%V(i-1:i,j,k))
               FQx(i,j,k,5)=FQx(i,j,k,1)*0.5_WP*sum(this%W(i-1:i,j,k))
               ! Velocity gradients at x-face
               gradU(1,1)=this%dxi*(this%U(i,j,k)-this%U(i-1,j,k))
               gradU(2,1)=0.25_WP*this%dyi*(this%U(i-1,j+1,k)-this%U(i-1,j-1,k)+this%U(i,j+1,k)-this%U(i,j-1,k))
               gradU(3,1)=0.25_WP*this%dzi*(this%U(i-1,j,k+1)-this%U(i-1,j,k-1)+this%U(i,j,k+1)-this%U(i,j,k-1))
               gradU(1,2)=this%dxi*(this%V(i,j,k)-this%V(i-1,j,k))
               gradU(2,2)=0.25_WP*this%dyi*(this%V(i-1,j+1,k)-this%V(i-1,j-1,k)+this%V(i,j+1,k)-this%V(i,j-1,k))
               gradU(3,2)=0.25_WP*this%dzi*(this%V(i-1,j,k+1)-this%V(i-1,j,k-1)+this%V(i,j,k+1)-this%V(i,j,k-1))
               gradU(1,3)=this%dxi*(this%W(i,j,k)-this%W(i-1,j,k))
               gradU(2,3)=0.25_WP*this%dyi*(this%W(i-1,j+1,k)-this%W(i-1,j-1,k)+this%W(i,j+1,k)-this%W(i,j-1,k))
               gradU(3,3)=0.25_WP*this%dzi*(this%W(i-1,j,k+1)-this%W(i-1,j,k-1)+this%W(i,j,k+1)-this%W(i,j,k-1))
               div=gradU(1,1)+gradU(2,2)+gradU(3,3)
               ! Viscous stress at x-face (added to momentum fluxes)
               FQx(i,j,k,3)=FQx(i,j,k,3)+0.5_WP*sum(this%VISC(i-1:i,j,k))*(gradU(1,1)+gradU(1,1))+0.5_WP*(sum(this%BETA(i-1:i,j,k))-2.0_WP/3.0_WP*sum(this%VISC(i-1:i,j,k)))*div
               FQx(i,j,k,4)=FQx(i,j,k,4)+0.5_WP*sum(this%VISC(i-1:i,j,k))*(gradU(2,1)+gradU(1,2))
               FQx(i,j,k,5)=FQx(i,j,k,5)+0.5_WP*sum(this%VISC(i-1:i,j,k))*(gradU(3,1)+gradU(1,3))
               ! Heat diffusion flux
               FQx(i,j,k,2)=FQx(i,j,k,2)+0.5_WP*sum(this%DIFF(i-1:i,j,k))*this%dxi*(this%T(i,j,k)-this%T(i-1,j,k))
               ...
               FQy(i,j,k,1)=-0.5_WP*(vel+abs(vel))*sum(wenop*this%Q(i,j-2:j  ,k,1))&
               &            -0.5_WP*(vel-abs(vel))*sum(wenom*this%Q(i,j-1:j+1,k,1))
               ! Centered mass flux
               ...
               ! Momentum flux and pressure stress
               FQy(i,j,k,3)=FQy(i,j,k,1)*0.5_WP*sum(this%U(i,j-1:j,k))
               FQy(i,j,k,4)=FQy(i,j,k,1)*0.5_WP*sum(this%V(i,j-1:j,k))-0.5_WP*sum(this%P(i,j-1:j,k))
               FQy(i,j,k,5)=FQy(i,j,k,1)*0.5_WP*sum(this%W(i,j-1:j,k))
               ! Velocity gradients at y-face
               gradU(1,1)=0.25_WP*this%dxi*(this%U(i+1,j-1,k)-this%U(i-1,j-1,k)+this%U(i+1,j,k)-this%U(i-1,j,k))
               gradU(2,1)=this%dyi*(this%U(i,j,k)-this%U(i,j-1,k))
               gradU(3,1)=0.25_WP*this%dzi*(this%U(i,j-1,k+1)-this%U(i,j-1,k-1)+this%U(i,j,k+1)-this%U(i,j,k-1))
               gradU(1,2)=0.25_WP*this%dxi*(this%V(i+1,j-1,k)-this%V(i-1,j-1,k)+this%V(i+1,j,k)-this%V(i-1,j,k))
               gradU(2,2)=this%dyi*(this%V(i,j,k)-this%V(i,j-1,k))
               gradU(3,2)=0.25_WP*this%dzi*(this%V(i,j-1,k+1)-this%V(i,j-1,k-1)+this%V(i,j,k+1)-this%V(i,j,k-1))
               gradU(1,3)=0.25_WP*this%dxi*(this%W(i+1,j-1,k)-this%W(i-1,j-1,k)+this%W(i+1,j,k)-this%W(i-1,j,k))
               gradU(2,3)=this%dyi*(this%W(i,j,k)-this%W(i,j-1,k))
               gradU(3,3)=0.25_WP*this%dzi*(this%W(i,j-1,k+1)-this%W(i,j-1,k-1)+this%W(i,j,k+1)-this%W(i,j,k-1))
               div=gradU(1,1)+gradU(2,2)+gradU(3,3)
               ! Viscous stress at y-face (added to momentum fluxes)
               FQy(i,j,k,3)=FQy(i,j,k,3)+0.5_WP*sum(this%VISC(i,j-1:j,k))*(gradU(1,2)+gradU(2,1))
               FQy(i,j,k,4)=FQy(i,j,k,4)+0.5_WP*sum(this%VISC(i,j-1:j,k))*(gradU(2,2)+gradU(2,2))+0.5_WP*(sum(this%BETA(i,j-1:j,k))-2.0_WP/3.0_WP*sum(this%VISC(i,j-1:j,k)))*div
               FQy(i,j,k,5)=FQy(i,j,k,5)+0.5_WP*sum(this%VISC(i,j-1:j,k))*(gradU(3,2)+gradU(2,3))
               ! Heat diffusion flux
               FQy(i,j,k,2)=FQy(i,j,k,2)+0.5_WP*sum(this%DIFF(i,j-1:j,k))*this%dyi*(this%T(i,j,k)-this%T(i,j-1,k))
               ...
               FQz(i,j,k,1)=-0.5_WP*(vel+abs(vel))*sum(wenop*this%Q(i,j,k-2:k  ,1))&
               &            -0.5_WP*(vel-abs(vel))*sum(wenom*this%Q(i,j,k-1:k+1,1))
               ! Centered mass flux
               ...
               ! Momentum flux and pressure stress
               FQz(i,j,k,3)=FQz(i,j,k,1)*0.5_WP*sum(this%U(i,j,k-1:k))
               FQz(i,j,k,4)=FQz(i,j,k,1)*0.5_WP*sum(this%V(i,j,k-1:k))
               FQz(i,j,k,5)=FQz(i,j,k,1)*0.5_WP*sum(this%W(i,j,k-1:k))-0.5_WP*sum(this%P(i,j,k-1:k))
               ! Velocity gradients at z-face
               gradU(1,1)=0.25_WP*this%dxi*(this%U(i+1,j,k-1)-this%U(i-1,j,k-1)+this%U(i+1,j,k)-this%U(i-1,j,k))
               gradU(2,1)=0.25_WP*this%dyi*(this%U(i,j+1,k-1)-this%U(i,j-1,k-1)+this%U(i,j+1,k)-this%U(i,j-1,k))
               gradU(3,1)=this%dzi*(this%U(i,j,k)-this%U(i,j,k-1))
               gradU(1,2)=0.25_WP*this%dxi*(this%V(i+1,j,k-1)-this%V(i-1,j,k-1)+this%V(i+1,j,k)-this%V(i-1,j,k))
               gradU(2,2)=0.25_WP*this%dyi*(this%V(i,j+1,k-1)-this%V(i,j-1,k-1)+this%V(i,j+1,k)-this%V(i,j-1,k))
               gradU(3,2)=this%dzi*(this%V(i,j,k)-this%V(i,j,k-1))
               gradU(1,3)=0.25_WP*this%dxi*(this%W(i+1,j,k-1)-this%W(i-1,j,k-1)+this%W(i+1,j,k)-this%W(i-1,j,k))
               gradU(2,3)=0.25_WP*this%dyi*(this%W(i,j+1,k-1)-this%W(i,j-1,k-1)+this%W(i,j+1,k)-this%W(i,j-1,k))
               gradU(3,3)=this%dzi*(this%W(i,j,k)-this%W(i,j,k-1))
               div=gradU(1,1)+gradU(2,2)+gradU(3,3)
               ! Viscous stress at z-face (added to momentum fluxes)
               FQz(i,j,k,3)=FQz(i,j,k,3)+0.5_WP*sum(this%VISC(i,j,k-1:k))*(gradU(1,3)+gradU(3,1))
               FQz(i,j,k,4)=FQz(i,j,k,4)+0.5_WP*sum(this%VISC(i,j,k-1:k))*(gradU(2,3)+gradU(3,2))
               FQz(i,j,k,5)=FQz(i,j,k,5)+0.5_WP*sum(this%VISC(i,j,k-1:k))*(gradU(3,3)+gradU(3,3))+0.5_WP*(sum(this%BETA(i,j,k-1:k))-2.0_WP/3.0_WP*sum(this%VISC(i,j,k-1:k)))*div
               ! Heat diffusion flux
               FQz(i,j,k,2)=FQz(i,j,k,2)+0.5_WP*sum(this%DIFF(i,j,k-1:k))*this%dzi*(this%T(i,j,k)-this%T(i,j,k-1))
            end do
         end do
      end do
      ...
      do k=this%cfg%kmin_,this%cfg%kmax_
         do j=this%cfg%jmin_,this%cfg%jmax_
            do i=this%cfg%imin_,this%cfg%imax_
               ! Mass, internal energy, and momentum advection
               ...
               dQdt(i,j,k,3)=this%dxi*(FQx(i+1,j,k,3)-FQx(i,j,k,3))+this%dyi*(FQy(i,j+1,k,3)-FQy(i,j,k,3))+this%dzi*(FQz(i,j,k+1,3)-FQz(i,j,k,3))
               dQdt(i,j,k,4)=this%dxi*(FQx(i+1,j,k,4)-FQx(i,j,k,4))+this%dyi*(FQy(i,j+1,k,4)-FQy(i,j,k,4))+this%dzi*(FQz(i,j,k+1,4)-FQz(i,j,k,4))
               dQdt(i,j,k,5)=this%dxi*(FQx(i+1,j,k,5)-FQx(i,j,k,5))+this%dyi*(FQy(i,j+1,k,5)-FQy(i,j,k,5))+this%dzi*(FQz(i,j,k+1,5)-FQz(i,j,k,5))
               ! Pressure dilatation term
               dQdt(i,j,k,2)=dQdt(i,j,k,2)-this%P(i,j,k)*0.5_WP*(this%dxi*(this%U(i+1,j,k)-this%U(i-1,j,k))+this%dyi*(this%V(i,j+1,k)-this%V(i,j-1,k))+this%dzi*(this%W(i,j,k+1)-this%W(i,j,k-1)))
               ...
               div=gradU(1,1)+gradU(2,2)+gradU(3,3)
               ! τ:∇U = τ_ij * gradU(i,j)
               dQdt(i,j,k,2)=dQdt(i,j,k,2) &
               & +(2.0_WP*this%VISC(i,j,k)*gradU(1,1)+(this%BETA(i,j,k)-2.0_WP/3.0_WP*this%VISC(i,j,k))*div)*gradU(1,1) &
               & +(2.0_WP*this%VISC(i,j,k)*gradU(2,2)+(this%BETA(i,j,k)-2.0_WP/3.0_WP*this%VISC(i,j,k))*div)*gradU(2,2) &
               & +(2.0_WP*this%VISC(i,j,k)*gradU(3,3)+(this%BETA(i,j,k)-2.0_WP/3.0_WP*this%VISC(i,j,k))*div)*gradU(3,3) &
               & +this%VISC(i,j,k)*(gradU(2,1)+gradU(1,2))*(gradU(2,1)+gradU(1,2)) &
               & +this%VISC(i,j,k)*(gradU(3,1)+gradU(1,3))*(gradU(3,1)+gradU(1,3)) &
               & +this%VISC(i,j,k)*(gradU(3,2)+gradU(2,3))*(gradU(3,2)+gradU(2,3))
            end do
         end do
      end do
      ...
      ! ! Allocate viscous heating term
      ! allocate(VFQ(this%cfg%imino_:this%cfg%imaxo_,this%cfg%jmino_:this%cfg%jmaxo_,this%cfg%kmino_:this%cfg%kmaxo_)); VFQ=0.0_WP
      ...
      !    FQx(i,j,k,3)=2.0_WP*this%VISC(i,j,k)*gradU(1,1)+(this%BETA(i,j,k)-2.0_WP*this%VISC(i,j,k)/3.0_WP)*(gradU(1,1)+gradU(2,2)+gradU(3,3))
      !    FQx(i,j,k,4)=this%VISC(i,j,k)*(gradU(2,1)+gradU(1,2))
      !    FQx(i,j,k,5)=this%VISC(i,j,k)*(gradU(3,1)+gradU(1,3))
      !    FQy(i,j,k,3)=this%VISC(i,j,k)*(gradU(2,1)+gradU(1,2))
      !    FQy(i,j,k,4)=2.0_WP*this%VISC(i,j,k)*gradU(2,2)+(this%BETA(i,j,k)-2.0_WP*this%VISC(i,j,k)/3.0_WP)*(gradU(1,1)+gradU(2,2)+gradU(3,3))
      !    FQy(i,j,k,5)=this%VISC(i,j,k)*(gradU(3,2)+gradU(2,3))
      !    FQz(i,j,k,3)=this%VISC(i,j,k)*(gradU(3,1)+gradU(1,3))
      !    FQz(i,j,k,4)=this%VISC(i,j,k)*(gradU(3,2)+gradU(2,3))
      !    FQz(i,j,k,5)=2.0_WP*this%VISC(i,j,k)*gradU(3,3)+(this%BETA(i,j,k)-2.0_WP*this%VISC(i,j,k)/3.0_WP)*(gradU(1,1)+gradU(2,2)+gradU(3,3))
      !    ! Assemble viscous heating source term
      !    VFQ(i,j,k)=FQx(i,j,k,3)*gradU(1,1)+FQx(i,j,k,4)*gradU(1,2)+FQx(i,j,k,5)*gradU(1,3)&
      !    &         +FQy(i,j,k,3)*gradU(2,1)+FQy(i,j,k,4)*gradU(2,2)+FQy(i,j,k,5)*gradU(2,3)&
      !    &         +FQz(i,j,k,3)*gradU(3,1)+FQz(i,j,k,4)*gradU(3,2)+FQz(i,j,k,5)*gradU(3,3)
      ...
      !    dQdt(i,j,k,3)=dQdt(i,j,k,3)+0.5_WP*(this%dxi*(FQx(i+1,j,k,3)-FQx(i-1,j,k,3))+this%dyi*(FQy(i,j+1,k,3)-FQy(i,j-1,k,3))+this%dzi*(FQz(i,j,k+1,3)-FQz(i,j,k-1,3)))
      !    dQdt(i,j,k,4)=dQdt(i,j,k,4)+0.5_WP*(this%dxi*(FQx(i+1,j,k,4)-FQx(i-1,j,k,4))+this%dyi*(FQy(i,j+1,k,4)-FQy(i,j-1,k,4))+this%dzi*(FQz(i,j,k+1,4)-FQz(i,j,k-1,4)))
      !    dQdt(i,j,k,5)=dQdt(i,j,k,5)+0.5_WP*(this%dxi*(FQx(i+1,j,k,5)-FQx(i-1,j,k,5))+this%dyi*(FQy(i,j+1,k,5)-FQy(i,j-1,k,5))+this%dzi*(FQz(i,j,k+1,5)-FQz(i,j,k-1,5)))
      !    dQdt(i,j,k,2)=dQdt(i,j,k,2)+VFQ(i,j,k)
      ! end do; end do; end do
   ...
   subroutine get_div_stress(this,divx,divy,divz,divq)
      ...
      do k=this%cfg%kmin_,this%cfg%kmax_+1
         do j=this%cfg%jmin_,this%cfg%jmax_+1
            do i=this%cfg%imin_,this%cfg%imax_+1
               ...
               gradU(3,3)=0.25_WP*this%dzi*(this%W(i-1,j,k+1)-this%W(i-1,j,k-1)+this%W(i,j,k+1)-this%W(i,j,k-1))
               div=gradU(1,1)+gradU(2,2)+gradU(3,3)
               ! Viscous and pressure stress at x-face (added to momentum fluxes)
               FQx(i,j,k,1)=0.5_WP*sum(this%VISC(i-1:i,j,k))*(gradU(1,1)+gradU(1,1))+0.5_WP*(sum(this%BETA(i-1:i,j,k))-2.0_WP/3.0_WP*sum(this%VISC(i-1:i,j,k)))*div-0.5_WP*sum(this%P(i-1:i,j,k))
               FQx(i,j,k,2)=0.5_WP*sum(this%VISC(i-1:i,j,k))*(gradU(2,1)+gradU(1,2))
               FQx(i,j,k,3)=0.5_WP*sum(this%VISC(i-1:i,j,k))*(gradU(3,1)+gradU(1,3))
               ! Fluxes in y
               ! Velocity gradients at y-face
               gradU(1,1)=0.25_WP*this%dxi*(this%U(i+1,j-1,k)-this%U(i-1,j-1,k)+this%U(i+1,j,k)-this%U(i-1,j,k))
               gradU(2,1)=this%dyi*(this%U(i,j,k)-this%U(i,j-1,k))
               ...
               gradU(3,3)=0.25_WP*this%dzi*(this%W(i,j-1,k+1)-this%W(i,j-1,k-1)+this%W(i,j,k+1)-this%W(i,j,k-1))
               div=gradU(1,1)+gradU(2,2)+gradU(3,3)
               ! Viscous and pressure stress at y-face (added to momentum fluxes)
               FQy(i,j,k,1)=0.5_WP*sum(this%VISC(i,j-1:j,k))*(gradU(1,2)+gradU(2,1))
               FQy(i,j,k,2)=0.5_WP*sum(this%VISC(i,j-1:j,k))*(gradU(2,2)+gradU(2,2))+0.5_WP*(sum(this%BETA(i,j-1:j,k))-2.0_WP/3.0_WP*sum(this%VISC(i,j-1:j,k)))*div-0.5_WP*sum(this%P(i,j-1:j,k))
               FQy(i,j,k,3)=0.5_WP*sum(this%VISC(i,j-1:j,k))*(gradU(3,2)+gradU(2,3))
               ! Fluxes in z
               ! Velocity gradients at z-face
               gradU(1,1)=0.25_WP*this%dxi*(this%U(i+1,j,k-1)-this%U(i-1,j,k-1)+this%U(i+1,j,k)-this%U(i-1,j,k))
               gradU(2,1)=0.25_WP*this%dyi*(this%U(i,j+1,k-1)-this%U(i,j-1,k-1)+this%U(i,j+1,k)-this%U(i,j-1,k))
               ...
               gradU(3,3)=this%dzi*(this%W(i,j,k)-this%W(i,j,k-1))
               div=gradU(1,1)+gradU(2,2)+gradU(3,3)
               ! Viscous and pressure stress at z-face (added to momentum fluxes)
               FQz(i,j,k,1)=0.5_WP*sum(this%VISC(i,j,k-1:k))*(gradU(1,3)+gradU(3,1))
               FQz(i,j,k,2)=0.5_WP*sum(this%VISC(i,j,k-1:k))*(gradU(2,3)+gradU(3,2))
               FQz(i,j,k,3)=0.5_WP*sum(this%VISC(i,j,k-1:k))*(gradU(3,3)+gradU(3,3))+0.5_WP*(sum(this%BETA(i,j,k-1:k))-2.0_WP/3.0_WP*sum(this%VISC(i,j,k-1:k)))*div-0.5_WP*sum(this%P(i,j,k-1:k))
               ! Heat fluxes
      ...
      do k=this%cfg%kmin_,this%cfg%kmax_
         do j=this%cfg%jmin_,this%cfg%jmax_
            do i=this%cfg%imin_,this%cfg%imax_
               ! Momentum components
               divx(i,j,k)=this%dxi*(FQx(i+1,j,k,1)-FQx(i,j,k,1))+this%dyi*(FQy(i,j+1,k,1)-FQy(i,j,k,1))+this%dzi*(FQz(i,j,k+1,1)-FQz(i,j,k,1))
               divy(i,j,k)=this%dxi*(FQx(i+1,j,k,2)-FQx(i,j,k,2))+this%dyi*(FQy(i,j+1,k,2)-FQy(i,j,k,2))+this%dzi*(FQz(i,j,k+1,2)-FQz(i,j,k,2))
               divz(i,j,k)=this%dxi*(FQx(i+1,j,k,3)-FQx(i,j,k,3))+this%dyi*(FQy(i,j+1,k,3)-FQy(i,j,k,3))+this%dzi*(FQz(i,j,k+1,3)-FQz(i,j,k,3))
      ...
      !          gradU(3,3)=0.5_WP*this%dzi*(this%W(i,j,k+1)-this%W(i,j,k-1))
      !          ! Assemble Newtonian stress tensor
      !          FQx(i,j,k,1)=2.0_WP*this%VISC(i,j,k)*gradU(1,1)+(this%BETA(i,j,k)-2.0_WP*this%VISC(i,j,k)/3.0_WP)*(gradU(1,1)+gradU(2,2)+gradU(3,3))
      !          FQx(i,j,k,2)=this%VISC(i,j,k)*(gradU(2,1)+gradU(1,2))
      !          FQx(i,j,k,3)=this%VISC(i,j,k)*(gradU(3,1)+gradU(1,3))
      !          FQy(i,j,k,1)=this%VISC(i,j,k)*(gradU(2,1)+gradU(1,2))
      !          FQy(i,j,k,2)=2.0_WP*this%VISC(i,j,k)*gradU(2,2)+(this%BETA(i,j,k)-2.0_WP*this%VISC(i,j,k)/3.0_WP)*(gradU(1,1)+gradU(2,2)+gradU(3,3))
      !          FQy(i,j,k,3)=this%VISC(i,j,k)*(gradU(3,2)+gradU(2,3))
      !          FQz(i,j,k,1)=this%VISC(i,j,k)*(gradU(3,1)+gradU(1,3))
      !          FQz(i,j,k,2)=this%VISC(i,j,k)*(gradU(3,2)+gradU(2,3))
      !          FQz(i,j,k,3)=2.0_WP*this%VISC(i,j,k)*gradU(3,3)+(this%BETA(i,j,k)-2.0_WP*this%VISC(i,j,k)/3.0_WP)*(gradU(1,1)+gradU(2,2)+gradU(3,3))
      !          Px (i,j,k)  = 0.5_WP*sum(this%P(i-1:i,j,k))
      !          Py (i,j,k)  = 0.5_WP*sum(this%P(i,j-1:j,k))
      !          Pz (i,j,k)  = 0.5_WP*sum(this%P(i,j,k-1:k))
      !          ! Heat fluxes
      !          FQx(i,j,k,4)=0.5_WP*(this%DIFF(i-1,j,k)+this%DIFF(i,j,k))*this%dxi*(this%T(i,j,k)-this%T(i-1,j,k))
      !          FQy(i,j,k,4)=0.5_WP*(this%DIFF(i,j-1,k)+this%DIFF(i,j,k))*this%dyi*(this%T(i,j,k)-this%T(i,j-1,k))
      !          FQz(i,j,k,4)=0.5_WP*(this%DIFF(i,j,k-1)+this%DIFF(i,j,k))*this%dzi*(this%T(i,j,k)-this%T(i,j,k-1))
      ...
      !    call this%cfg%sync(FQz(:,:,:,i))
      ! end do
      ! call this%cfg%sync(Px); call this%cfg%sync(Py); call this%cfg%sync(Pz) 

      ! ! Assemble divergence of of stresses
      ! do k=this%cfg%kmin_,this%cfg%kmax_
      !    do j=this%cfg%jmin_,this%cfg%jmax_
      !       do i=this%cfg%imin_,this%cfg%imax_
      !          ! Viscous momentum transport
      !          divx(i,j,k)=0.5_WP*(this%dxi*(FQx(i+1,j,k,1)-FQx(i-1,j,k,1))+this%dyi*(FQy(i,j+1,k,1)-FQy(i,j-1,k,1))+this%dzi*(FQz(i,j,k+1,1)-FQz(i,j,k-1,1)))+(this%dxi*(Px(i+1,j,k)-Px(i,j,k)))
      !          divy(i,j,k)=0.5_WP*(this%dxi*(FQx(i+1,j,k,2)-FQx(i-1,j,k,2))+this%dyi*(FQy(i,j+1,k,2)-FQy(i,j-1,k,2))+this%dzi*(FQz(i,j,k+1,2)-FQz(i,j,k-1,2)))+(this%dyi*(Py(i,j+1,k)-Py(i,j,k)))
      !          divz(i,j,k)=0.5_WP*(this%dxi*(FQx(i+1,j,k,3)-FQx(i-1,j,k,3))+this%dyi*(FQy(i,j+1,k,3)-FQy(i,j-1,k,3))+this%dzi*(FQz(i,j,k+1,3)-FQz(i,j,k-1,3)))+(this%dzi*(Px(i,j,k+1)-Pz(i,j,k)))
      !          divq(i,j,k)=this%dxi*(FQx(i+1,j,k,4)-FQx(i  ,j,k,4))+this%dyi*(FQy(i,j+1,k,4)-FQy(i,j  ,k,4))+this%dzi*(FQz(i,j,k+1,4)-FQz(i,j,k  ,4))
      !       end do
      !    end do
      ! end do
      ...
      ! Deallocate flux arrays
      deallocate(FQx,FQy,FQz) !,Px,Py,Pz)
      ...
      do k=this%cfg%kmino_+1,this%cfg%kmaxo_
         do j=this%cfg%jmino_+1,this%cfg%jmaxo_
            do i=this%cfg%imino_+1,this%cfg%imaxo_
               this%U(i,j,k)=this%Q(i,j,k,3)/this%Q(i,j,k,1)
               this%V(i,j,k)=this%Q(i,j,k,4)/this%Q(i,j,k,1)
               this%W(i,j,k)=this%Q(i,j,k,5)/this%Q(i,j,k,1)
            end do
         end do
      end do
      ! Sync velocity
      ...
      do k=this%cfg%kmino_,this%cfg%kmaxo_-1
         do j=this%cfg%jmino_,this%cfg%jmaxo_-1
            do i=this%cfg%imino_,this%cfg%imaxo_-1
               KE(i,j,k)=0.5_WP*(this%U(i,j,k)**2+this%V(i,j,k)**2+this%W(i,j,k)**2)
            end do
         end do
      end do
   ...
   subroutine get_momentum(this)
      ...
      do k=this%cfg%kmino_+1,this%cfg%kmaxo_
         do j=this%cfg%jmino_+1,this%cfg%jmaxo_
            do i=this%cfg%imino_+1,this%cfg%imaxo_
               this%Q(i,j,k,3)=this%Q(i,j,k,1)*this%U(i,j,k)
               this%Q(i,j,k,4)=this%Q(i,j,k,1)*this%V(i,j,k)
               this%Q(i,j,k,5)=this%Q(i,j,k,1)*this%W(i,j,k)
            end do
         end do
      end do
      ! Sync momentum
