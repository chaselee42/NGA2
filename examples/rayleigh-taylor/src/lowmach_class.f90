...
module lowmach_class
   ...
   contains
      ...
      procedure :: get_viscosity                          !< Calculate viscosity fields using viscosity interpolation
...
contains
   ...
   function constructor(cfg,name) result(self)
      ...
      allocate(self%Umid(self%cfg%imino_:self%cfg%imaxo_,self%cfg%jmino_:self%cfg%jmaxo_,self%cfg%kmino_:self%cfg%kmaxo_)); self%Umid=0.0_WP
      allocate(self%Vmid(self%cfg%imino_:self%cfg%imaxo_,self%cfg%jmino_:self%cfg%jmaxo_,self%cfg%kmino_:self%cfg%kmaxo_)); self%Vmid=0.0_WP
      allocate(self%Wmid(self%cfg%imino_:self%cfg%imaxo_,self%cfg%jmino_:self%cfg%jmaxo_,self%cfg%kmino_:self%cfg%kmaxo_)); self%Wmid=0.0_WP
      
       ! Mass conservation data around which to build momentum/energy conservation
      allocate(self%sRHOX   (self%cfg%imino_:self%cfg%imaxo_,self%cfg%jmino_:self%cfg%jmaxo_,self%cfg%kmino_:self%cfg%kmaxo_)); self%sRHOX=0.0_WP
      allocate(self%sRHOY   (self%cfg%imino_:self%cfg%imaxo_,self%cfg%jmino_:self%cfg%jmaxo_,self%cfg%kmino_:self%cfg%kmaxo_)); self%sRHOY=0.0_WP
      allocate(self%sRHOZ   (self%cfg%imino_:self%cfg%imaxo_,self%cfg%jmino_:self%cfg%jmaxo_,self%cfg%kmino_:self%cfg%kmaxo_)); self%sRHOZ=0.0_WP
      allocate(self%sRHOXold(self%cfg%imino_:self%cfg%imaxo_,self%cfg%jmino_:self%cfg%jmaxo_,self%cfg%kmino_:self%cfg%kmaxo_)); self%sRHOXold=0.0_WP
      allocate(self%sRHOYold(self%cfg%imino_:self%cfg%imaxo_,self%cfg%jmino_:self%cfg%jmaxo_,self%cfg%kmino_:self%cfg%kmaxo_)); self%sRHOYold=0.0_WP
      allocate(self%sRHOZold(self%cfg%imino_:self%cfg%imaxo_,self%cfg%jmino_:self%cfg%jmaxo_,self%cfg%kmino_:self%cfg%kmaxo_)); self%sRHOZold=0.0_WP
   ...
   subroutine get_viscosity(this)
      ...
      class(lowmach), intent(inout) :: this
      integer :: i,j,k
      ! Compute viscosity field on velocity-cell
      do k=this%cfg%kmino_+1,this%cfg%kmaxo_
         do j=this%cfg%jmino_+1,this%cfg%jmaxo_
            do i=this%cfg%imino_+1,this%cfg%imaxo_
               ! VISC_xy at [x,y,zm] - direct sum in z, staggered sum in x/y
               this%visc_xy(i,j,k)=sum(this%itp_xy(:,:,i  ,j,k)*this%visc(i-1:i,j-1:j,k))
               ! VISC_yz at [xm,y,z] - direct sum in x, staggered sum in y/z
               this%visc_yz(i,j,k)=sum(this%itp_yz(:,:,i,j,k  )*this%visc(i,j-1:j,k-1:k))
               ! VISC_zx at [x,ym,z] - direct sum in y, staggered sum in z/x
               this%visc_zx(i,j,k)=sum(this%itp_xz(:,:,i  ,j,k)*this%visc(i-1:i,j,k-1:k))
            end do
         end do
      end do
