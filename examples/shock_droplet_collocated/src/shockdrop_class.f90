...
contains
   ...
   subroutine analyze_drops(this)
      ...
      do n=1,this%ccl%nstruct
         ...
         do m=1,this%ccl%struct(n)%n_
            ...
            Ud(1,n)=Ud(1,n)+this%fs%cfg%vol(i,j,k)*this%fs%Q (i,j,k,1)*this%fs%U(i,j,k)
            Ud(2,n)=Ud(2,n)+this%fs%cfg%vol(i,j,k)*this%fs%Q (i,j,k,1)*this%fs%V(i,j,k)
            Ud(3,n)=Ud(3,n)+this%fs%cfg%vol(i,j,k)*this%fs%Q (i,j,k,1)*this%fs%W(i,j,k)
         end do
      end do
   ...
   subroutine initialize(this,dx,meshsize,startloc,group,partition,continue_monitor)
      ...
      create_config: block
         ...
         grid=sgrid(coord=cartesian,no=3,x=x,y=y,z=z,xper=xper,yper=xper,zper=zper,name='ShockDrop')
         ! Deallocate x/y/z
         deallocate(x,y,z)
         ! Create config
   ...
   subroutine step(this,dt)
      ...
      ! Remember old velocities
      this%fs%Uold=this%fs%U; this%fs%Vold=this%fs%V; this%fs%Wold=this%fs%W;
   ...
   subroutine finalize(this)
      ...
      deallocate(this%dQdt,this%Ma,this%beta,this%visc)
