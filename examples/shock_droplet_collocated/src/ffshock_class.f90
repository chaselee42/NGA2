...
module ffshock_class
   ...
   contains
      ...
      procedure :: interp_velface                  !< Interpolate velocity to cell face for lpt solver
...
contains
   ...
   subroutine step(this,dt)
      ...
      ! Get cell face velocities
      call this%interp_velface()
      ! Advance particles
   ...
   subroutine apply_bconds(this)
      ...
      if (.not.this%fs%cfg%xper.and.this%fs%cfg%iproc.eq.this%fs%cfg%npx) then
         do k=this%fs%cfg%kmino_,this%fs%cfg%kmaxo_; do j=this%fs%cfg%jmino_,this%fs%cfg%jmaxo_
            ...
            do i=this%fs%cfg%imax,this%fs%cfg%imaxo
      ...
      if (.not.this%fs%cfg%yper.and.this%fs%cfg%jproc.eq.this%fs%cfg%npy) then
         do k=this%fs%cfg%kmino_,this%fs%cfg%kmaxo_; do i=this%fs%cfg%imino_,this%fs%cfg%imaxo_
            ...
            do j=this%fs%cfg%jmax,this%fs%cfg%jmaxo
               ! Copy primitive variables
      ...
      if (.not.this%fs%cfg%yper.and.this%fs%cfg%jproc.eq.1) then
         do k=this%fs%cfg%kmino_,this%fs%cfg%kmaxo_; do i=this%fs%cfg%imino_,this%fs%cfg%imaxo_
            ...
            do j=this%fs%cfg%jmino,this%fs%cfg%jmin
               ! Copy primitive variables
      ...
      if (.not.this%fs%cfg%zper.and.this%fs%cfg%kproc.eq.this%fs%cfg%npz) then
         do j=this%fs%cfg%jmino_,this%fs%cfg%jmaxo_; do i=this%fs%cfg%imino_,this%fs%cfg%imaxo_
            ...
            do k=this%fs%cfg%kmax,this%fs%cfg%kmaxo
               ! Copy primitive variables
      ...
      if (.not.this%fs%cfg%zper.and.this%fs%cfg%kproc.eq.1) then
         do j=this%fs%cfg%jmino_,this%fs%cfg%jmaxo_; do i=this%fs%cfg%imino_,this%fs%cfg%imaxo_
            ...
            do k=this%fs%cfg%kmino,this%fs%cfg%kmin
               ! Copy primitive variables
   ...
   !> Interpolate cell center velocities to cell face for lpt
   subroutine interp_velface(this)
      implicit none
      class(ffshock), intent(inout) :: this
      integer :: i,j,k
      ! Zero out cell center velocities
      this%Ui=0.0_WP; this%Vi=0.0_WP; this%Wi=0.0_WP
      ! Loop through domain for the interpolation
      do k=this%fs%cfg%kmin_,this%fs%cfg%kmax_+1; do j=this%fs%cfg%jmin_,this%fs%cfg%jmax_+1; do i=this%fs%cfg%imin_,this%fs%cfg%imax_+1
         this%Ui(i,j,k)=0.5_WP*sum(this%fs%U(i-1:i,j,k));
         this%Vi(i,j,k)=0.5_WP*sum(this%fs%V(i,j-1:j,k));
         this%Wi(i,j,k)=0.5_WP*sum(this%fs%W(i,j,k-1:k));
      end do; end do; end do
      ! Sync quantities
      call this%fs%cfg%sync(this%Ui); call this%fs%cfg%sync(this%Vi); call this%fs%cfg%sync(this%Wi); 
   end subroutine interp_velface
