...
contains
   ...
   subroutine simulation_init
      ...
      initialize_sd: block
         ...
         do k=sd%cfg%kmino_,sd%cfg%kmaxo_; do j=sd%cfg%jmino_,sd%cfg%jmaxo_; do i=sd%cfg%imino_,sd%cfg%imaxo_
            ...
            sd%fs%U(i,j,k)=u2*Hshock(Xs-sd%fs%cfg%xm(i),delta=0.5_WP*sd%fs%dx)
         ...
         sd%Ma=sqrt(sd%fs%U**2+sd%fs%V**2+sd%fs%W**2)/sd%fs%C
         ! Perform monitoring
      ...
      initialize_ff: block
         ...
         do k=ff%cfg%kmino_,ff%cfg%kmaxo_; do j=ff%cfg%jmino_,ff%cfg%jmaxo_; do i=ff%cfg%imino_,ff%cfg%imaxo_
            ff%fs%U(i,j,k)  =u2*Hshock(Xs-ff%fs%cfg%xm(i),delta=0.5_WP*ff%fs%dx)
         ...
         ! Compute local Mach number
         ff%Ma=sqrt(ff%fs%U**2+ff%fs%V**2+ff%fs%W**2)/ff%fs%C
   ...
   subroutine simulation_run
      ...
      do while (.not.time%done())
         ...
         ! call ff%fs%get_cfl(dt=time%dt,cfl=time%cfl)
         call time%adjust_dt()
         call time%increment()
         ! Handle coupling
         call tcpl%start()
   ...
   subroutine remesh()
      ...
      initialize_sdnew_from_ff: block
         ...
         call ff2sdnew%push(ff%fs%Q(:,:,:,3),loc='c'); call ff2sdnew%transfer(); call ff2sdnew%pull(sdnew%fs%Q(:,:,:,5),loc='c')
         call ff2sdnew%push(ff%fs%Q(:,:,:,4),loc='c'); call ff2sdnew%transfer(); call ff2sdnew%pull(sdnew%fs%Q(:,:,:,6),loc='c')
         call ff2sdnew%push(ff%fs%Q(:,:,:,5),loc='c'); call ff2sdnew%transfer(); call ff2sdnew%pull(sdnew%fs%Q(:,:,:,7),loc='c')
      ...
      initialize_sdnew_from_sd: block
         ...
         call sd2sdnew%push(sd%fs%Q(:,:,:,5),loc='c'); call sd2sdnew%transfer(); call sd2sdnew%pull(sdnew%fs%Q(:,:,:,5),loc='c')
         call sd2sdnew%push(sd%fs%Q(:,:,:,6),loc='c'); call sd2sdnew%transfer(); call sd2sdnew%pull(sdnew%fs%Q(:,:,:,6),loc='c')
         call sd2sdnew%push(sd%fs%Q(:,:,:,7),loc='c'); call sd2sdnew%transfer(); call sd2sdnew%pull(sdnew%fs%Q(:,:,:,7),loc='c')
         ! Transfer VF
         ...
         ! Compute local Mach number
         sdnew%Ma=sqrt(sdnew%fs%U**2+sdnew%fs%V**2+sdnew%fs%W**2)/sdnew%fs%C
   ...
   subroutine couple_sd2ff()
      ...
      call sd2ff%push(sd%fs%U   ,loc='c'); call sd2ff%transfer(); call sd2ff%pull(Utmp  ,loc='c')
      call sd2ff%push(sd%fs%V   ,loc='c'); call sd2ff%transfer(); call sd2ff%pull(Vtmp  ,loc='c')
      call sd2ff%push(sd%fs%W   ,loc='c'); call sd2ff%transfer(); call sd2ff%pull(Wtmp  ,loc='c')
      do k=ff%cfg%kmino_,ff%cfg%kmaxo_; do j=ff%cfg%jmino_,ff%cfg%jmaxo_; do i=ff%cfg%imino_,ff%cfg%imaxo_
         ...
         ff%fs%U(i,j,k)=ff%fs%U(i,j,k)+coeffc*(Utmp(i,j,k)-ff%fs%U(i,j,k))
         ff%fs%V(i,j,k)=ff%fs%V(i,j,k)+coeffc*(Vtmp(i,j,k)-ff%fs%V(i,j,k))
         ff%fs%W(i,j,k)=ff%fs%W(i,j,k)+coeffc*(Wtmp(i,j,k)-ff%fs%W(i,j,k))
      end do; end do; end do
   ...
   subroutine couple_ff2sd()
      ...
      call ff2sd%push(ff%fs%U         ,loc='c'); call ff2sd%transfer(); call ff2sd%pull(Utmp   ,loc='c')
      call ff2sd%push(ff%fs%V         ,loc='c'); call ff2sd%transfer(); call ff2sd%pull(Vtmp   ,loc='c')
      call ff2sd%push(ff%fs%W         ,loc='c'); call ff2sd%transfer(); call ff2sd%pull(Wtmp   ,loc='c')
      do k=sd%cfg%kmino_,sd%cfg%kmaxo_; do j=sd%cfg%jmino_,sd%cfg%jmaxo_; do i=sd%cfg%imino_,sd%cfg%imaxo_
         ! Compute cell-centered nudging coefficient
         ...
         ! Apply nudging on velocity
         sd%fs%U(i,j,k)=sd%fs%U(i,j,k)+coeffc*(Utmp(i,j,k)-sd%fs%U(i,j,k))
         sd%fs%V(i,j,k)=sd%fs%V(i,j,k)+coeffc*(Vtmp(i,j,k)-sd%fs%V(i,j,k))
         sd%fs%W(i,j,k)=sd%fs%W(i,j,k)+coeffc*(Wtmp(i,j,k)-sd%fs%W(i,j,k))
      end do; end do; end do
   ...
   subroutine transfer()
      ...
      real(WP), parameter :: vol_coeff=32.0_WP            !< excessively large drops are not transfered
      ...
      do n=1,sd%ccl%nstruct
         ...
         do m=1,sd%ccl%struct(n)%n_
            ...
            Ud(1,n)=Ud(1,n)+sd%fs%cfg%vol(i,j,k)*sd%fs%VF(i,j,k)*sd%fs%U(i,j,k)
            Ud(2,n)=Ud(2,n)+sd%fs%cfg%vol(i,j,k)*sd%fs%VF(i,j,k)*sd%fs%V(i,j,k)
            Ud(3,n)=Ud(3,n)+sd%fs%cfg%vol(i,j,k)*sd%fs%VF(i,j,k)*sd%fs%W(i,j,k)
         end do
      end do
